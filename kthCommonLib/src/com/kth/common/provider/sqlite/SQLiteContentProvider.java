...
public abstract class SQLiteContentProvider extends ContentProvider implements
    ...
    private static final int URI_INDEX_INSERTED = 0;

    private static final int URI_INDEX_UPDATED = 1;
     ...
     * transaction.
     */
    // protected abstract Uri insertInTransaction(Uri uri, ContentValues
    // values);
    ...
    protected abstract void notifyChange(Uri uri);
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        if (!applyingBatch) {
            ...
            try {
                Uri[] subResult = insertInTransaction(uri, values);
                if (subResult != null && subResult.length >= 2) {
                    if (subResult[URI_INDEX_INSERTED] != null)
                        result = subResult[URI_INDEX_INSERTED];
                    else if (subResult[URI_INDEX_UPDATED] != null)
                        result = subResult[URI_INDEX_UPDATED];
                }
    ...
    public int bulkInsert(Uri uri, ContentValues[] values) {
        ...
        try {
            for (int i = 0; i < numValues; i++) {
                Uri[] subResult = insertInTransaction(uri, values[i]);
